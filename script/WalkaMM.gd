...
func enemy_turn():
	...
	if efekt_czaru_aktywny:
		...
		if current_enemy_health <= 0:
			...
			#wygrana, powrót do eksploracji
			self.hide()
			Globalsy.fight=false
			Globalsy.dialogue_active=false
			Globalsy.MM_ded=true
			Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
			#ustawienie
			State.current_health = current_player_health
			State.current_mana = current_player_mana
...
func _on_ATTACK_pressed():
	...
	if current_enemy_health <= 0:
		...
		State.current_health = current_player_health
		State.current_mana = current_player_mana
		get_parent().queue_free()#usunięcie postaci Mięsoludka
		#queue_free()
		#get_tree().change_scene_to_file("res://Assety/scenes/menu/start_menu.tscn")
	enemy_turn()
